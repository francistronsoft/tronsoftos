...
app.post('/api/databases/:id/disable-indexes', { preHandler: requireOperator }, async (req, reply) => {
  ...
  try {
    ...
    const cmd = [
      ...
      'echo "[5/8] Restaurando backup em arquivo temporario com indices inativos" >> "$log"',
      ...
      `${gbak} -c -inactive -v -user SYSDBA -password ${password} "$restore_src" ${shQuote(firebirdCreateTarget(restoredPath))} >> "$log" 2>&1 || fail 67 "Falha ao restaurar banco sem indices ativos"`,
      'rm -f "$restore_src"',
